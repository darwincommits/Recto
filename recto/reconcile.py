...
Replaces the imperative `nssm set <service> AppPath ...` PowerShell
...
PowerShell sequence:
    ...
    nssm set verso AppPath C:\\Python312\\python.exe
...
- AppPath -- the python.exe (or other interpreter) NSSM invokes.
...
def compute_plan(
    ...
    *,
    ...
    # DisplayName falls back to the service name if no description set;
    # NSSM shows DisplayName in the Services snap-in. Description is the
    # longer "what is this service for" string.
    desired_display_name = cfg.metadata.description or cfg.metadata.name
    desired_description = cfg.metadata.description

    field_changes = (
        FieldChange("AppPath", current.app_path, desired_app_path),
        FieldChange(
