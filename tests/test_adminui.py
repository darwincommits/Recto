...
class TestAdminUIServerLifecycle:
    ...
    def test_soft_fail_on_bind_error(self) -> None:
        # Try to bind to a privileged port (likely to fail under
        # non-root) or use an obviously-invalid host. We use a
        # malformed bind string -- _parse_bind defaults but the
        # server.bind on 127.0.0.1:5050 might succeed; safer: use
        # 0.0.0.0:1 (port 1 is privileged) or grab port 0 first then
        # try to re-bind same port.
        # Simpler: bind once, then attempt a SECOND server on the
        # same address. The second start() must soft-fail.
        spec = AdminUISpec(enabled=True, bind="127.0.0.1:0")
        buf = EventBuffer()
        cfg = make_config()
        first = AdminUIServer(
